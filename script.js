...
function renderPlayers(players) {
    ...
    // Renderizar por bloques de 5 en orden original, numerando globalmente
    rankingContainer.innerHTML = Array.from({ length: Math.ceil(players.length / 5) }, (_, i) => {
        const group = players.slice(i * 5, (i + 1) * 5);
        ...
        return `
            <div class="tier-card">
                <div class="tier-header">Grupo ${i + 1}</div>
                <div class="tier-body" style="border: 4px solid ${border}">
                    ${group.map((player, idx) => `
                        <div class="player-card">
                            <div class="player-header">
                                <div class="player-rank">${startIndex + idx + 1}</div>
                                <div>
